...
<p align="center">
  免责声明
  本软件（微信自动回复系统）仅供学习、研究和合法用途。使用者须自行承担以下责任：
1. 本软件不得用于发送垃圾消息、诈骗、骚扰或其他违反法律法规的行为。
2. 使用本软件可能违反微信相关服务条款，由此导致的账号封禁等后果由使用者自行承担。
3. 开发者不对因使用本软件而产生的任何直接或间接损失负责。
4. 使用第三方 API 时，请妥善保管 API Key，避免泄露。API 调用产生的费用由使用者自行承担。
</p>
